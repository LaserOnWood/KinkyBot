...
class Moderation(commands.Cog):
    ...
    async def kick(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison"):
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="Je n'ai pas les permissions pour expulser ce membre.", color=0xE74C3C)
    ...
    async def ban(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison"):
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="Je n'ai pas les permissions pour bannir ce membre.", color=0xE74C3C)
            await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def unban(self, interaction: discord.Interaction, user_id: str):
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="ID invalide ou erreur lors du débannissement.", color=0xE74C3C)
            await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def mute(self, interaction: discord.Interaction, membre: discord.Member, minutes: int = 10):
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="Je n'ai pas les permissions pour mute ce membre.", color=0xE74C3C)
            await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def clear(self, interaction: discord.Interaction, nombre: int):
        ...
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="Je n'ai pas les permissions pour supprimer des messages.", color=0xE74C3C)
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def move_messages(self, interaction: discord.Interaction, salon: discord.TextChannel, nombre: int):
        ...
        try:
            ...
            embed = discord.Embed(title="❌ Erreur", description="Permissions insuffisantes pour effectuer cette action.", color=0xE74C3C)
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def cog_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        # On vérifie si une réponse a déjà été envoyée ou différée
        ...
        if isinstance(error, app_commands.MissingPermissions):
            ...
            embed = discord.Embed(title="❌ Erreur", description="Je n'ai pas les permissions nécessaires pour faire ça.", color=0xE74C3C)
            await send_func(embed=embed, ephemeral=True)
        else:
            embed = discord.Embed(title="❌ Erreur", description=f"Une erreur est survenue : {error}", color=0xE74C3C)
            await send_func(embed=embed, ephemeral=True)
